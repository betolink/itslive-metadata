...
LABEL org.opencontainers.image.description="ITS_LIVE granle metadata generator"
...
COPY --chown=${CONDA_UID}:${CONDA_GID} . /hyp3-itslive-metadata/

RUN mamba env create -f /hyp3-itslive-metadata/environment.yml && \
    conda clean -afy && \
    conda activate hyp3-itslive-metadata && \
    sed -i 's/conda activate base/conda activate hyp3-itslive-metadata/g' /home/conda/.profile && \
